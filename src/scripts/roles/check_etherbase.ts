...
async function CheckEtherbase(controller: GlobalController, chain: string) {
    ...
    try {
        ...
        const ETHER_MANAGER_ROLE = await contract.callStatic.ETHER_MANAGER_ROLE();

        /// Check Originator
        const originator_is_default_admin_role = await contract.callStatic.hasRole(DEFAULT_ADMIN_ROLE, public_key);
        const originator_is_ether_manager_role = await contract.callStatic.hasRole(ETHER_MANAGER_ROLE, public_key);

        result['originator']['originator_is_default_admin_role'] = originator_is_default_admin_role;
        result['originator']['originator_is_ether_manager_role'] = originator_is_ether_manager_role;

        /// Check Safe
        const safe_is_default_admin_role = await contract.callStatic.hasRole(DEFAULT_ADMIN_ROLE, safe_key);
        const safe_is_ether_manager_role = await contract.callStatic.hasRole(ETHER_MANAGER_ROLE, safe_key);

        result['safe']['safe_is_default_admin_role'] = safe_is_default_admin_role;
        result['safe']['safe_is_ether_manager_role'] = safe_is_ether_manager_role;
